...
echo "→ Locale files..."
"$PYTHON" scripts/validate_locales.py || fail "locale validation failed"
green "locales OK"

echo "→ Packaging checks..."
"$PYTHON" -m pytest -q tests/test_docker_packaging.py tests/test_i18n.py || fail "packaging/i18n tests failed"
